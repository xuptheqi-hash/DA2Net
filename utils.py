...
def get_loader(data_name, img_size=256, batch_size=8, split='test',
    ...
    root_dir = dataConfig.root_dir + "\\test"
...
def get_loaders(args):
    ...
    dataConfig = data_config.DataConfig().get_data_config(data_name)
    train_root_dir = dataConfig.root_dir + r'/train'
    test_root_dir = dataConfig.root_dir + r'/test'
    label_transform = dataConfig.label_transform
    split = args.split
    split_val = 'val'
    ...
    if args.dataset == 'CDDataset':
        training_set = CDDataset(root_dir=train_root_dir, split=split,
                                 img_size=args.img_size,is_train=True,
                                 label_transform=label_transform)
        val_set = CDDataset(root_dir=test_root_dir, split=split_val,
